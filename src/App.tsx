...
import {
  ...
  ChevronLeft, ArrowUpDown, CheckSquare, ListPlus, FolderPlus, Shuffle, Pencil,
} from 'lucide-react';
...
import {
  notificationsSupported, notificationPermission, requestNotificationPermission, showBackgroundNotification,
  ...
  return (
    ...
    <div className="fixed bottom-[192px] md:bottom-24 left-1/2 -translate-x-1/2 z-50 animate-toast-slide-up flex items-center gap-2.5"
      style={{
        ...
        padding: '12px 22px', color: 'white', fontSize: 14, fontWeight: 500, whiteSpace: 'nowrap',
        boxShadow: `0 12px 36px -8px rgba(0,0,0,0.65), 0 0 0 1px ${accentColor}20, 0 0 24px -4px ${accentColor}40`,
      }}>
...
export default function App() {
  ...
  const loadAll = useCallback(async () => {
    ...
    setOsNotifications((prefs.osNotifications ?? false) && notificationPermission() === 'granted');
      ...
      {showBulkPlaylistMenu && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
          ...
          <div className="w-full max-w-xs rounded-2xl p-4 shadow-2xl animate-slide-up"
            style={{ background: 'linear-gradient(180deg, rgba(255,255,255,0.05), rgba(255,255,255,0) 30%), rgba(18,18,21,0.96)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div className="max-h-60 overflow-y-auto space-y-1 mb-2">
              {manualPlaylists.length === 0 && <p className="text-white/30 text-xs py-2">No playlists yet</p>}
              {manualPlaylists.map((pl) => (
                <button key={pl.id} onClick={() => handleBulkAddToPlaylist(pl.id)}
                  ...
                  <span className="text-white/30 text-xs shrink-0">{playlistCounts[pl.id] ?? pl.songIds.length}</span>
                </button>
              ))}
            </div>
