...
export function SettingsPanel({
  accentColor, onAccentChange, onClose, autoTheme, onToggleAutoTheme, rowSize, onRowSizeChange, songCount, onDeleteAllSongs, onRescanArt, artRescan,
  ...
  osNotifications, onToggleOSNotifications, notificationsSupported, notifPermission,
}: Props) {
    ...
    <div ref={overlayRef} className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-sm max-h-[85vh] overflow-y-auto rounded-2xl p-6 shadow-2xl animate-slide-up"
        ...
        <button
          ...
          className="w-full flex items-center justify-between gap-2 py-2.5 px-3 rounded-xl border border-white/10 text-sm font-medium transition-colors hover:bg-white/5 mb-2.5"
        >
          <span className="flex items-center gap-2 text-white/70 text-left">
            <ImagePlus size={15} />
            <span>
              Auto theme from album art
              <span className="block text-[11px] font-normal text-white/35 mt-0.5">Accent color follows what's playing</span>
            </span>
          </span>
          <span className="w-9 h-5 rounded-full relative transition-colors shrink-0" style={{ background: autoTheme ? accentColor : 'rgba(255,255,255,0.15)' }}>
            <span className="absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all" style={{ left: autoTheme ? 18 : 2 }} />
          </span>
        </button>
        ...
        {colorExpanded && (
          <div className="mb-1">
            ...
            <div className="flex items-center gap-2 mt-3">
              ...
              <div className="w-10 h-10 rounded-xl border border-white/20 shrink-0" style={{ background: draftColor }} />
            </div>
        ...
        <div className="mt-5 pt-4 border-t border-white/10">
          <h3 className="text-white/60 text-xs font-semibold uppercase tracking-wider mb-3">Row Size</h3>
        ...
        <div className="mt-5 pt-4 border-t border-white/10">
          ...
          >
            <span className="text-left">
              <span className="text-white/70 text-sm font-medium block">Background notifications</span>
              <span className="text-white/30 text-xs block mt-0.5">
                {!notificationsSupported
                  ? "Not available in this browser."
                  : notifPermission === 'denied'
                  ? 'Blocked — enable notifications for this site in your browser settings.'
                  : 'Get an OS notification when a background scan finds new songs.'}
              </span>
            </span>
            <span className="w-9 h-5 rounded-full relative transition-colors shrink-0" style={{ background: osNotifications ? accentColor : 'rgba(255,255,255,0.15)' }}>
              <span className="absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all" style={{ left: osNotifications ? 18 : 2 }} />
            </span>
          </button>
        </div>
      ...
      {confirmingDeleteAll && (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center px-4"
          ...
          <div className="w-full max-w-sm rounded-2xl p-6 shadow-2xl animate-slide-up"
            style={{ background: 'linear-gradient(180deg, rgba(255,255,255,0.05), rgba(255,255,255,0) 30%), rgba(20,20,23,0.96)', backdropFilter: 'blur(20px)', border: '1px solid rgba(239,68,68,0.25)', boxShadow: '0 8px 32px rgba(0,0,0,0.6)' }}>
