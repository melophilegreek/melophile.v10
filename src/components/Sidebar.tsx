...
export function Sidebar({
  ...
  return (
    <div className="h-full flex flex-col py-4 px-3 overflow-y-auto" style={{ background: '#101013' }}>
      ...
      <div className="flex items-center justify-between px-2 mb-2">
        ...
        <div className="flex items-center gap-0.5">
          {onCreateSmartPlaylist && (
            <button onClick={onCreateSmartPlaylist} className="btn-icon w-6 h-6 hover:bg-white/10 rounded-md" title="New smart playlist">
          ...
          return (
            <div key={pl.id} className="relative flex items-center group rounded-lg transition-colors cursor-pointer"
              ...
              <div className="flex-1 flex items-center gap-2.5 px-3 py-2 min-w-0">
                {pl.smart ? (
                  <Sparkles size={16} style={{ color: active ? accentColor : 'rgba(255,255,255,0.4)', flexShrink: 0 }} />
                ) : (
                ...
                <span className="text-xs text-white/30 shrink-0">{playlistCounts?.[pl.id] ?? pl.songIds.length}</span>
              </div>
              {hovered && (
